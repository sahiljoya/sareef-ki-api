...
function isFileValid(filename, mimetype) {
    var allowedExts = ["mp4", "webm", "ogg"]; var allowedMimeTypes = ["video/mp4", "video/webm", "video/ogg"];
    // Get file extension.  var extension = getExtension(filename);  
    return allowedExts.indexOf(extension.toLowerCase()) != -1 && allowedMimeTypes.indexOf(mimetype) != -1;
}
function upload(req, callback) {  // The route on which the file is saved.
    ...
    // Used for sending response.  var link = null;   
    // Stream error handler. 
    function handleStreamError(error) {  // Do not enter twice in here. 
        if (hadStreamError) { return; }
        hadStreamError = error;
        // Cleanup: delete the saved path.
        if (saveToPath) {
            return fs.unlink(saveToPath, function (err) { return callback(error); });
    ...
    busboy.on("file", function (fieldname, file, filename, encoding, mimetype) {
        ...
        if ("file" != fieldname) {
            ...
            file.resume(); return callback("Fieldname is not correct. It must be " + file + ".");
        }
        // Generate link. 
        var randomName = sha1(new Date().getTime()) + "." + getExtension(filename); link = fileRoute + randomName;
        ...
        diskWriterStream.on("finish", function () {
            ...
            if (!status) { return handleStreamError("File does not meet the validation."); }
